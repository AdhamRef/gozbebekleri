    ...
    <form onSubmit={submitProject} className="rounded-lg border bg-white p-4 shadow-sm">
      <div className="flex flex-col gap-2 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm leading-6 text-slate-600">سجل المشروع حتى يتم ربط ملفاته ومراجعة مواده داخل الأرشيف.</p>
        </div>
      ...
      <div className="mt-3 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        <label className="grid gap-1 text-xs font-bold text-slate-600">
          البلد
          <input value={country} onChange={(event) => setCountry(event.target.value)} placeholder="فلسطين" className="h-10 rounded-md border bg-white px-3 text-sm text-slate-900 outline-none focus:border-[#025EB8]" />
        </label>
        <label className="grid gap-1 text-xs font-bold text-slate-600">
          المدينة
          <input value={city} onChange={(event) => setCity(event.target.value)} placeholder="غزة / القدس" className="h-10 rounded-md border bg-white px-3 text-sm text-slate-900 outline-none focus:border-[#025EB8]" />
        </label>
        <label className="grid gap-1 text-xs font-bold text-slate-600">
          التصنيف
          <input value={theme} onChange={(event) => setTheme(event.target.value)} placeholder="مياه / وقف / زكاة" className="h-10 rounded-md border bg-white px-3 text-sm text-slate-900 outline-none focus:border-[#025EB8]" />
        </label>
        <label className="grid gap-1 text-xs font-bold text-slate-600">
          نوع المشروع
          <input value={projectType} onChange={(event) => setProjectType(event.target.value)} placeholder="إغاثة طارئة" className="h-10 rounded-md border bg-white px-3 text-sm text-slate-900 outline-none focus:border-[#025EB8]" />
        </label>
      </div>
