  ...
  return (
    <section className="rounded-lg border bg-white p-4 shadow-sm" aria-label="إضافة إطار رسالة">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <p className="text-xs font-bold text-[#025EB8]">قوالب الرسائل</p>
          <h2 className="mt-1 text-lg font-black text-slate-950">إضافة قالب رسالة للحملات</h2>
          <p className="mt-1 text-sm leading-6 text-slate-600">يُحفظ كمسودة تحتاج مراجعة بشرية قبل الاعتماد.</p>
        </div>
      ...
      <form onSubmit={submit} className="mt-4 grid gap-3">
        ...
        <div className="grid gap-3 lg:grid-cols-2">
          ...
          <textarea value={doText} onChange={(event) => setDoText(event.target.value)} placeholder="المسموح: قاعدة في كل سطر" aria-label="قائمة المسموح" className="min-h-24 rounded-md border border-input bg-white px-3 py-2 text-sm shadow-sm outline-none transition focus-visible:ring-2 focus-visible:ring-ring" />
          <textarea value={dontText} onChange={(event) => setDontText(event.target.value)} placeholder="الممنوع: قاعدة في كل سطر" aria-label="قائمة الممنوع" className="min-h-24 rounded-md border border-input bg-white px-3 py-2 text-sm shadow-sm outline-none transition focus-visible:ring-2 focus-visible:ring-ring" />
        </div>
      </form>
