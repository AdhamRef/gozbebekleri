...
export default function UsersPage() {
  ...
  const [sortBy, setSortBy] = useState<"createdAt" | "name" | "email" | "donationsCount" | "totalDonated">("createdAt");
  ...
  const fetchUsers = useCallback(
    async (pageNum: number, append: boolean) => {
      ...
      try {
        ...
        if (preferredLangFilter && preferredLangFilter !== "all") params.set("preferredLang", preferredLangFilter);
        if (badgeFilter && badgeFilter !== "all") params.set("badgeId", badgeFilter);
    ...
    [search, roleFilter, preferredLangFilter, badgeFilter, sortBy, sortOrder]
  );
  ...
  useEffect(() => {
    setPage(1);
    fetchUsers(1, false);
  }, [search, roleFilter, preferredLangFilter, badgeFilter, sortBy, sortOrder, fetchUsers]);

  useEffect(() => {
    axios.get("/api/admin/badges?locale=ar").then((res) => {
  ...
  const handleUserUpdate = async (newRole: string) => {
  ...
  return (
    <div className="min-h-0" dir="rtl">
      <div className="space-y-6 sm:space-y-8 p-0 sm:p-4 md:p-6 lg:p-8 max-w-[1600px] mx-auto">
        <header className="text-right">
          ...
          <p className="text-muted-foreground mt-1 text-sm">
            عرض وإدارة المستخدمين، اللغة المفضلة وإجمالي التبرعات
          </p>
        </header>
        ...
        <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <Card className="border border-border shadow-sm" dir="rtl">
            <div className="p-5 flex items-start gap-4">
              <div className="p-2.5 rounded-lg shrink-0 bg-violet-50 text-violet-600 border border-violet-200">
        ...
        />
              ...
              <div className="space-y-2 text-right">
                <label className="text-[11px] font-medium text-slate-500">الدور</label>
                <Select value={roleFilter} onValueChange={setRoleFilter}>
                  ...
                  <SelectContent>
                    <SelectItem value="all" className="text-xs">الكل</SelectItem>
                    <SelectItem value="DONOR" className="text-xs">متبرع</SelectItem>
      ...
      <Dialog open={isDialogOpen} onOpenChange={(open) => !open && handleDialogClose()}>
        ...
        <DialogContent className="fixed top-1/2 left-1/2 w-[calc(100%-2rem)] max-w-md p-4 sm:p-6 transform -translate-x-1/2 -translate-y-1/2 bg-card text-card-foreground border border-border rounded-lg shadow-lg" dir="rtl">
          <DialogTitle className="text-lg font-bold text-foreground">تعديل الدور</DialogTitle>
          {selectedUser && (
            <div className="space-y-4 mt-4">
              ...
              <div className="flex justify-end gap-2 pt-2">
                ...
                <Button onClick={() => handleUserUpdate(selectedUser.role)}>
                  حفظ الدور
                </Button>
              </div>
            </div>
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <span className={cn(
                        ...
                        viewUser.role === "ADMIN" ? "bg-amber-500/90 text-white" : "bg-white/20 text-white"
                      )}>
                        {viewUser.role === "ADMIN" ? "مدير" : "متبرع"}
                      </span>
                    </div>
                  </div>
                </div>
