...
export default async function AdAccountsPage() {
  ...
  return <main className="space-y-8 p-4 sm:p-6" dir="rtl">
    ...
    <Card><CardHeader title="حالة المنصات الإعلانية" description={connectedCount === 0 ? "لا توجد حسابات مربوطة بعد." : `${connectedCount} حساب مربوط.`} action={<Link href="#manage" className="text-xs font-bold text-brand">إدارة الحسابات</Link>} /><div className="grid gap-3 p-4 lg:grid-cols-2">{rows.map((row) => <div key={row.key} className="rounded-xl border p-4"><div className="flex items-center justify-between gap-2"><span className="font-black">{row.label}</span><StatusBadge status={row.status} /></div><div className="mt-3 grid gap-1.5 text-xs text-slate-500"><div className="flex justify-between"><span>مربوط</span><span>{row.connected ? "نعم" : "لا"}</span></div><div className="flex justify-between"><span>اكتمال الإعداد</span><span>{row.completionPercent}%</span></div><div className="flex justify-between"><span>آخر مزامنة</span><span>{fmtDate(row.lastSyncAt)}</span></div>{row.lastError ? <div className="mt-1 rounded-md bg-rose-50 px-2 py-1 text-rose-700">تعذر آخر تشغيل. راجع السجلات الآمنة.</div> : null}</div></div>)}</div></Card>
