...
export default async function PlatformConnectionsOverview() {
  ...
  return <main className="space-y-6 p-4 sm:p-6" dir="rtl">
    ...
    <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
      ...
      <StatusCard title="مزودو التواصل والإرسال" status={communicationStatus} actionHref={`${BASE}/communication`} actionLabel="فتح المزودين" detail={<><p>واتساب: {STATUS_LABEL[activeStatus(meta)]}</p><p>الإيميل: {STATUS_LABEL[activeStatus(brevo)]}</p><p>SMS: {STATUS_LABEL[activeStatus(netgsm)]}</p><p>Cron: {scheduler.configured ? "مُفعّل" : "يحتاج إعداد"}</p></>} />
