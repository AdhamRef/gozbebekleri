...
import { AlertTriangle, ArrowRight, CheckCircle2, Download, Loader2, RefreshCw, TrendingDown, TrendingUp, Wrench } from "lucide-react";
...
export default function BudgetRecommendationsPage() {
  ...
  return <div className="space-y-6 p-4 sm:p-6" dir="rtl">
    ...
    {loading ? <div className="flex min-h-[20rem] items-center justify-center"><Loader2 className="h-8 w-8 animate-spin text-[#025EB8]" /></div> : !data ? <Card><CardContent className="p-8 text-center text-slate-500">لا توجد بيانات.</CardContent></Card> : <>
      <div className="grid grid-cols-1 gap-3 md:grid-cols-6"><Kpi label="إجمالي" value={String(data.summary.total)} /><Kpi label="زود" value={String(data.summary.scale)} /><Kpi label="أوقف" value={String(data.summary.pause)} /><Kpi label="خفّض" value={String(data.summary.reduce)} /><Kpi label="أصلح التتبع" value={String(data.summary.fixTracking)} /><Kpi label="Site ROAS" value={`${data.summary.siteRoas.toFixed(2)}x`} /></div>
      <Card><CardHeader><CardTitle>قائمة القرارات</CardTitle><CardDescription>هذه التوصيات تعتمد على البيانات المتاحة، ولا تغني عن مراجعة جودة الإعلان والجمهور قبل قرارات كبيرة.</CardDescription></CardHeader><CardContent className="space-y-3">{data.recommendations.length === 0 ? <div className="rounded-xl bg-slate-50 p-8 text-center text-slate-500">لا توجد توصيات بعد. أدخل أو استورد بيانات المنصات أولًا.</div> : data.recommendations.map((rec) => <div key={rec.id} className="rounded-xl border bg-white p-4 shadow-sm"><div className="flex flex-wrap items-start justify-between gap-3"><div className="max-w-4xl"><span className={`inline-flex items-center gap-1 rounded-full border px-2 py-1 text-xs font-medium ${decisionClass(rec.decision)}`}>{icon(rec.decision)}{decisionLabel(rec.decision)}</span><h2 className="mt-2 text-base font-bold text-slate-950">{rec.title}</h2><p className="mt-1 text-sm leading-6 text-slate-600">{rec.reason}</p><div className="mt-3 rounded-lg bg-slate-50 p-3 text-sm text-slate-800"><b>الإجراء:</b> {rec.action}</div><div className="mt-2 flex flex-wrap gap-2 text-xs text-slate-500"><span>Spend: {money(rec.metrics.spend)}</span><span>Site Revenue: {money(rec.metrics.siteRevenue)}</span><span>Site ROAS: {(rec.metrics.siteRoas || 0).toFixed(2)}x</span><span>تبرعات الموقع: {rec.metrics.siteDonations || 0}</span><span>تحويلات المنصة: {rec.metrics.platformConversions || 0}</span></div></div><Link href={rec.href} className="rounded-md border px-3 py-2 text-sm hover:bg-slate-50">فتح المصدر</Link></div></div>)}</CardContent></Card>
    </>}
  </div>;
}
