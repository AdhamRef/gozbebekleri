...
export default function MarketingIntelligencePage() {
  ...
  const load = React.useCallback(async (targetDays = days) => {
    ...
    try {
      ...
      toast.error("تعذر تحميل بيانات الإعلانات والتتبع");
      setData(null);
    } finally {
      setLoading(false);
    }
  ...
  return <div className="space-y-5 p-4 sm:p-6" dir="rtl">
    <div className="rounded-3xl border bg-gradient-to-l from-[#025EB8] to-[#01396f] p-6 text-white shadow-sm">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <h1 className="mt-2 text-3xl font-black">الإعلانات والتتبع</h1>
          <p className="mt-2 max-w-3xl text-sm leading-7 text-white/85">لوحة واحدة مختصرة لمعرفة الصرف، التبرعات، ROAS، الحسابات الإعلانية، وآخر مزامنة. التفاصيل الفنية مخفية وتفتح عند الحاجة فقط.</p>
        </div>
        <div className="flex flex-wrap gap-2">
          ...
          <Button variant="secondary" onClick={() => load(days)} className="gap-2"><RefreshCw className="h-4 w-4" />تحديث</Button>
        </div>
      </div>
    </div>
    ...
    {loading ? <div className="flex min-h-[22rem] items-center justify-center"><Loader2 className="h-8 w-8 animate-spin text-[#025EB8]" /></div> : !data ? <Card><CardContent className="p-8 text-center text-slate-500">لا توجد بيانات متاحة.</CardContent></Card> : <>
      ...
      <div className="grid gap-4 xl:grid-cols-[1.2fr_1fr]">
        <Card>
          <CardHeader>
            ...
            <CardDescription>ملخص المنصات المتصلة وما تم سحبه من بيانات الإنفاق والتحويلات.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-3">
            {data.platforms.length === 0 ? <Empty text="لا توجد بيانات صرف مسحوبة بعد. افتح ربط المنصات أو استورد بيانات المنصات." /> : data.platforms.map((row) => <div key={`${row.platform}-${row.currency ?? ""}`} className="rounded-xl border p-3">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <div className="font-bold text-slate-950">{row.platform}</div>
                <div className="font-mono text-sm font-bold text-[#025EB8]">{money(row.spend, row.currency ?? "USD")}</div>
              </div>
              <div className="mt-2 grid gap-2 text-xs text-slate-500 md:grid-cols-4">
                <span>ظهور: {number(row.impressions)}</span><span>نقرات: {number(row.clicks)}</span><span>تحويلات: {number(row.conversions)}</span><span>قيمة: {money(row.revenue, row.currency ?? "USD")}</span>
              </div>
            </div>)}
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader>
            <CardTitle>حالة الربط والمزامنة</CardTitle>
            <CardDescription>أهم شيء: هل الحسابات موجودة؟ وهل آخر مزامنة نجحت؟</CardDescription>
          </CardHeader>
          <CardContent className="space-y-3">
            ...
            <div className="flex flex-wrap gap-2 pt-2">
              <Link href="/dashboard/marketing/connections" className="inline-flex items-center gap-2 rounded-md border px-3 py-2 text-sm hover:bg-slate-50"><Settings className="h-4 w-4" />ربط الحسابات</Link>
              <Link href="/dashboard/marketing-intelligence/platform-metrics/import" className="inline-flex items-center gap-2 rounded-md border px-3 py-2 text-sm hover:bg-slate-50"><Database className="h-4 w-4" />استيراد البيانات</Link>
            </div>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid gap-4 xl:grid-cols-[1.2fr_1fr]">
        <Card>
          <CardHeader>
            <CardTitle>أهم الحملات حسب الصرف</CardTitle>
            <CardDescription>قائمة مختصرة بدل عشرات الصفحات. افتح التفاصيل فقط إذا احتجت.</CardDescription>
          </CardHeader>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2"><AlertTriangle className="h-5 w-5 text-amber-600" />تنبيهات مختصرة</CardTitle>
            <CardDescription>لا نعرض كل التفاصيل، فقط ما يحتاج انتباه.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-3">
            {data.summary.spend === 0 ? <Notice tone="warn" text="لا يوجد صرف ظاهر في الفترة الحالية. قد تحتاج مزامنة Meta أو استيراد بيانات المنصات." /> : null}
            {data.summary.totalConnections === 0 ? <Notice tone="warn" text="لا توجد حسابات إعلانية مربوطة. اربط الحسابات حتى تظهر بيانات الصرف تلقائيًا." /> : null}
            ...
            <div className="flex flex-wrap gap-2 pt-2">
              <Link href="/dashboard/marketing-intelligence/executive-overview" className="inline-flex items-center gap-2 rounded-md border px-3 py-2 text-sm hover:bg-slate-50"><CheckCircle2 className="h-4 w-4" />فتح التفاصيل الفنية</Link>
              <Link href="/dashboard/link-generator" className="inline-flex items-center gap-2 rounded-md border px-3 py-2 text-sm hover:bg-slate-50"><Zap className="h-4 w-4" />منشئ الروابط</Link>
            </div>
          </CardContent>
        </Card>
      </div>
