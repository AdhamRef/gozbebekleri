    ...
    const fetchStats = async () => {
      try {
        ...
        if (!res.ok) {
          ...
          toast.error(data?.details || data?.error || "فشل في تحميل إحصائيات الرابط");
          return;
        }
        setStats(data);
      } catch {
                    ...
                    {campaigns
                      .filter(
                        (c) =>
                          ...
                          (!searchCampaign || c.title.toLowerCase().includes(searchCampaign.toLowerCase()))
                      )
                      .map((c) => (
                      ...
                      chartMetric === "amount" ? (
                        <ResponsiveContainer width="100%" height="100%">
                          <BarChart data={chartData} barCategoryGap="16%" barGap={6} margin={{ top: 10, right: 24, left: 10, bottom: 0 }}>
                            ...
                            <XAxis dataKey="date" tick={{ fill: CHART_COLORS.text, fontSize: 12 }} tickFormatter={(v) => new Date(v).toLocaleDateString("en-US", { day: "numeric", month: "short" })} interval="preserveStartEnd" />
                            <YAxis tick={{ fill: CHART_COLORS.text, fontSize: 11 }} tickFormatter={(v) => formatMoney(Number(v))} domain={[0, "auto"]} />
                            <Tooltip
                              contentStyle={{ backgroundColor: "#fff", border: "1px solid #e2e8f0", borderRadius: "8px", boxShadow: "0 4px 6px -1px rgb(0 0 0 / 0.1)" }}
                              labelFormatter={(v) => new Date(v).toLocaleDateString("en-US", { weekday: "long", year: "numeric", month: "long", day: "numeric" })}
                              ...
                              content={({ active, payload, label }) => {
                                ...
                                return (
                                  <div className="rounded-lg border border-border bg-card px-3 py-2 shadow-sm">
                                    <p className="text-sm font-medium text-slate-700 mb-1.5">{label ? new Date(label).toLocaleDateString("ar-EG", { weekday: "long", year: "numeric", month: "long", day: "numeric" }) : ""}</p>
                                    {payload.map((entry) => (
