...
export default function PixelsSettingsPage() {
  ...
  function TestBox({ platform }: { platform: Platform }) {
    return <div className="rounded-lg border bg-slate-50 p-3 text-xs text-slate-600"><div className="grid gap-1 md:grid-cols-2"><span>Browser Pixel status: Placeholder</span><span>Server API status: Placeholder</span><span>Last conversion sent: Placeholder</span><span>Last error: Placeholder</span></div><div className="mt-2 border-t pt-2">آخر اختبار: {lastTest[platform] ?? "لم يتم اختباره"}</div></div>;
  }
  ...
  return <div className="space-y-6" dir="rtl">
    <div><h1 className="text-2xl font-bold">إعدادات البكسلات والتتبع</h1><p className="mt-1 text-muted-foreground">إدارة معرفات البكسل وواجهات التحويل من الخادم، واختبار جاهزية التتبع لكل منصة.</p></div>
