      ...
      <section className="grid gap-4 xl:grid-cols-2">
        {providers.map((provider) => (
          <Card key={provider.key} className="overflow-hidden">
            <CardHeader><div className="flex flex-wrap items-start justify-between gap-3"><div><CardDescription>{provider.channel}</CardDescription><CardTitle className="mt-1 flex items-center gap-2 text-lg"><PlugZap className="h-5 w-5 text-[#025EB8]" /> {provider.name}</CardTitle></div><Badge variant="outline" className={statusClass(provider.status)}>{statusLabel(provider.status)}</Badge></div></CardHeader>
            <CardContent className="space-y-4">
              <div><div className="flex items-center justify-between text-xs font-bold text-slate-500"><span>جاهزية الربط</span><span>{provider.configuredRequiredCount}/{provider.requiredCount}</span></div><div className="mt-2 h-2 overflow-hidden rounded-full bg-slate-100"><div className="h-full rounded-full bg-[#025EB8]" style={{ width: `${provider.readinessPercent}%` }} /></div></div>
