      ...
      <section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <h1 className="mt-1 text-xl sm:text-2xl font-bold text-slate-900 tracking-tight">سجل الإرسال</h1>
                  ...
                  {items.map((d) => (
                    <React.Fragment key={d.id}>
                      <tr className="border-b last:border-0">
                        ...
                        <td className="p-3 text-left"><button className="text-xs text-brand underline" onClick={() => setOpen(open === d.id ? null : d.id)}>{open === d.id ? "إخفاء" : "المحتوى"}</button></td>
                      </tr>
                      {open === d.id ? (
                        <tr className="bg-slate-50"><td colSpan={7} className="p-3">
                          <div className="rounded-lg border bg-white p-3 text-sm">
                            <p className="text-xs text-slate-400">{d.templateName ? `القالب: ${d.templateName} · ` : ""}{d.campaignId ? <Link href={`/dashboard/operations/communication/campaigns/${d.campaignId}`} className="text-brand underline">الحملة</Link> : "بدون حملة"}</p>
