...
export default async function CommunicationSettingsPage() {
  const [runtime, metaWebhook, scheduler, sendersWithNumber, lastEvent] = await Promise.all([
    ...
    getSchedulerStatus(),
    process.env.DATABASE_URL ? prisma.communicationSender.count({ where: { channel: "WHATSAPP", enabled: true, phoneNumberId: { not: null } } }).catch(() => 0) : 0,
  ...
  const providerCards = [
    ...
    { key: "BREVO_EMAIL", label: "Brevo Email", desc: "الإيميل — المزود الأساسي", cfg: runtime.brevoEmail },
