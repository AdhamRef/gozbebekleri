  ...
  return (
    <div className="space-y-5 p-4 sm:p-6" dir="rtl">
      <div className="flex flex-col gap-4 rounded-2xl border bg-slate-950 p-5 text-white shadow-sm lg:flex-row lg:items-center lg:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-4xl text-sm leading-6 text-white/80">
            هذه الصفحة تثبت الموديلات التي يمكن تحويلها إلى Prisma بدون تغيير الدفع أو التتبع أو إرسال أي بيانات لمنصات خارجية.
          </p>
        </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
        <Card>
          <CardHeader>
            <CardDescription>إجمالي الموديلات</CardDescription>
            <CardTitle className="text-3xl">{snapshot.summary.totalModels}</CardTitle>
          </CardHeader>
        </Card>
        <Card>
          <CardHeader>
            <CardDescription>جاهز لـ Prisma</CardDescription>
            <CardTitle className="text-3xl">{snapshot.summary.prismaReadyModels}</CardTitle>
          </CardHeader>
        </Card>
        <Card>
          <CardHeader>
            <CardDescription>DB backed الآن</CardDescription>
            <CardTitle className="text-3xl">{snapshot.summary.dbBackedModels}</CardTitle>
          </CardHeader>
        </Card>
        <Card>
          <CardHeader>
            <CardDescription>Foundation فقط</CardDescription>
            <CardTitle className="text-3xl">{snapshot.summary.foundationModels}</CardTitle>
          </CardHeader>
        </Card>
        <Card>
          <CardHeader>
            <CardDescription>Low risk</CardDescription>
            <CardTitle className="text-3xl">{snapshot.summary.lowRiskModels}</CardTitle>
          </CardHeader>
        </Card>
      </div>
      ...
      <Card className="border-emerald-200 bg-emerald-50/60">
        <CardHeader className="gap-3 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <CardDescription className="mt-2 leading-6 text-slate-700">
              العقود هنا تعريفية وتجهيزية فقط. لا توجد migration كبيرة، ولا إرسال تلقائي، ولا أسرار أمامية، ولا تغيير في runtime المالي أو التتبعي.
            </CardDescription>
          </div>
        ...
        <CardContent className="grid gap-2 md:grid-cols-2 xl:grid-cols-3">
          {snapshot.safetyRules.map((rule) => (
            <div key={rule} className="rounded-xl border border-emerald-100 bg-white p-3 text-sm font-semibold text-slate-700">
              {rule}
            </div>
          ))}
        </CardContent>
            ...
            <CardHeader className="gap-3 lg:flex-row lg:items-center lg:justify-between">
              <div>
                ...
                <CardDescription className="mt-2">
                  {group.total} models, {group.prismaReady} ready for Prisma, {group.dbBacked} DB-backed.
                </CardDescription>
              </div>
            ...
            <CardContent className="grid gap-3 xl:grid-cols-2">
              {group.contracts.map((contract) => (
                <div key={contract.name} className="rounded-2xl border bg-slate-50 p-4">
                  ...
                  <div className="mt-4 grid gap-3 md:grid-cols-2">
                    <div>
                      <p className="text-xs font-bold text-slate-500">Key fields</p>
                      <p className="mt-1 text-xs leading-5 text-slate-600">{contract.fields.slice(0, 10).join(", ")}</p>
                    </div>
                    <div>
                      <p className="text-xs font-bold text-slate-500">Indexes</p>
                      <p className="mt-1 text-xs leading-5 text-slate-600">{contract.indexes.join(", ")}</p>
                    </div>
                  </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            <Database className="h-5 w-5 text-[#025EB8]" /> الحزمة التالية المقترحة
          </CardTitle>
        ...
        <CardContent className="space-y-3">
          ...
          <div className="flex flex-wrap gap-2">
            {snapshot.nextRecommendedPackage.firstSlice.map((model) => (
              <Badge key={model} variant="outline" className="bg-slate-50">
                {model}
              </Badge>
            ))}
          </div>
