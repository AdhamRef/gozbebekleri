...
export async function GET(request: NextRequest) {
  try {
    ...
    const where = {
      ...(campaignId && { items: { some: { campaignId } } }), // Filter by campaignId in DonationItem
    ...
    const donations = await prisma.donation.findMany({
      ...
      include: {
        donor: {
          select: {
            id: true, // Include donor's ID
            name: true, // Include donor's name
            email: true,
            image: true,
          },
        },
        items: {
          include: {
            campaign: {
              select: {
                id: true, // Include campaign's ID
                title: true, // Include campaign's title
                images: true,
              },
            },
          },
    ...
    // Format the response to include donor and campaign details
