...
import { pendingSettingActivationSchema } from "@/lib/integration-settings/api-contracts";
...
export async function POST(request: NextRequest, context: RouteContext) {
  ...
  const parsed = pendingSettingActivationSchema.safeParse(await request.json().catch(() => null));
  if (!parsed.success) return NextResponse.json({ error: "إصدار القيمة المعلقة غير صالح." }, { status: 400 });

  try {
    const snapshot = await integrationSettingsService.activatePendingSetting(
      provider,
      key,
