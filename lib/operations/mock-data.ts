...
export const operationWeeklyThemes = [
...
export function getOperationsKpis() {
  return {
    openSeasons: operationSeasons.length,
    activePlans: operationPlans.length,
    contentItems: 39,
    openProductionTasks: operationTasks.length,
    readyForMarketing: operationItems.filter((item) => item.status === "APPROVED").length,
  };
}
