...
const CartPaymentDialog = ({
  ...
  // Admin kill-switch (GlobalSettings.payforEnabled) forces Stripe for every
  // currency, including TRY, when off.
  const use3D = payforEnabled && getCurrency() === "TRY" && !fallbackClientSecret;
    ...
    if (step.title === t("teamSupport")) return (
      <div className="space-y-5">
        ...
        <div dir={dir} className="flex justify-between gap-4">
          ...
          <Button onClick={handleNext} disabled={teamSupport === null} className="flex-1 bg-[#025EB8] hover:bg-[#014fa0] text-white inline-flex items-center justify-center gap-2">{nextLabel}</Button>
        </div>
      </div>
    );
